...
def get_film_detail(slug):
    data = api_get(f"/film/{slug}")
...
def classify_film(film):
    """Phân loại phim lẻ / phim bộ dựa trên total_episodes."""
...
async def process_film_with_cache(slug, film_name, cache, semaphore):
    """Xử lý một phim, dùng cache để tránh lấy stream lại."""
    async with semaphore:
        ...
        try:
            ...
            # Lấy stream URL từ tập đầu tiên của server đầu tiên
            first_ep = episodes[0].get("items", [])[0] if episodes[0].get("items") else None
            if not first_ep:
...
async def process_genre(genre, cache, semaphore):
    """Xử lý một thể loại: lấy phim, phân loại, lấy stream, tạo M3U8."""
    slug = genre["slug"]
    name = genre["name"]
    ...
    # Fetch all films for this genre
    all_films = []
    for page in range(1, max_pages + 1):
        print(f"  Fetching page {page}...")
        try:
            data = api_get(f"/films/the-loai/{slug}", params={"page": page})
    ...
    print(f"  Phân loại: {len(phim_le)} phim lẻ, {len(phim_bo)} phim bộ, {len(unknown)} không xác định")
    ...
    for r in results_le:
        if isinstance(r, dict):
            phim_le_results.append(r)
    
    for r in results_bo:
        if isinstance(r, dict):
            phim_bo_results.append(r)
    
    # Generate M3U8 files
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    
    if phim_le_results:
        output_path = os.path.join(OUTPUT_DIR, f"{slug}_phim-le.m3u8")
        build_m3u8(phim_le_results, output_path, f"Phim lẻ - {name}")
        print(f"  ✓ {output_path} ({len(phim_le_results)} phim)")
    
    if phim_bo_results:
        output_path = os.path.join(OUTPUT_DIR, f"{slug}_phim-bo.m3u8")
        build_m3u8(phim_bo_results, output_path, f"Phim bộ - {name}")
        print(f"  ✓ {output_path} ({len(phim_bo_results)} phim)")
    
    if not phim_le_results and not phim_bo_results:
        print(f"  ✗ Không tạo được file nào.")


def build_m3u8(streams, output_path, group_title=""):
    """Tạo file M3U8 chuẩn cho TV/IPTV."""
    lines = ["#EXTM3U"]
